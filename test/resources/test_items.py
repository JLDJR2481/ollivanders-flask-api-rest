...
def test_createItem(client):
    ...
    assert response.status_code == 400
    assert response.json == {"ERROR" : "Ya existe un item en el inventario con ese id. Introduzca una id distinta"}
...
def test_updateItem(client):
    ...
    assert response.status_code == 404
    assert response.json == {"ERROR": "No se ha encontrado un item con la id correspondiente"}
