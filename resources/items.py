...
def welcomeItem():
    return jsonify({"Bienvenido a la sección de items!": "Puedes realizar 4 operaciones: \nCrear un item con /create. \nBuscar un item con /find/id. Debes insertar una id. \nUpdatear un item con /update/id. Debes insertar una id. \nBorrar un item de la base de datos con /delete/id. Debes insertar una id."}), 200
...
def createItem():
    ...
    if not all(key in item for key in required_keys):
        ...
        if Services.readItem(item["_id"]) == None:
            ...
            return jsonify({"ERROR" : "Ya existe un item en el inventario con ese id. Introduzca una id distinta"}), 400
...
def updateItem(id):
    ...
    if Services.readItem(item_id) is None:
        return jsonify({"ERROR": "No se ha encontrado un item con la id correspondiente"}), 404
